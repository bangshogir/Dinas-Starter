    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 gap-9 lg:grid-cols-2">
            ...
            <div class="mb-4">
                        ...
                        <div class="mt-1 flex justify-center px-6 pt-10 pb-10 border-2 border-gray-300 dark:border-gray-700 border-dashed rounded-lg relative hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group" id="drop-area">
                            <div class="space-y-2 text-center relative z-10 bg-white/80 dark:bg-gray-900/80 p-4 rounded-xl backdrop-blur-sm shadow-sm">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-500">Klik untuk mengganti gambar</p>
                            </div>
                            <!-- Existing Image -->
                            <img id="preview" src="{{ asset('assets/' . $product->image) }}" class="absolute inset-0 w-full h-full object-cover rounded-lg opacity-60 group-hover:opacity-40 transition-opacity" />
                        </div>
                        @error('image')
                            <p class="text-theme-xs text-error-500 mt-1.5">{{ $message }}</p>
                        @enderror
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="seller_phone" value="{{ old('seller_phone', $product->seller_phone) }}" required
                            class="shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                    </div>
                </div>
            </div>
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" rows="5"
                            class="shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('description', $product->description) }}</textarea>
                    </div>
